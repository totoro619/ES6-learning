...
// console.log(lesson4.A,lesson4.Hello);
// lesson4.test();
...
var Base = function () {
	...
	_createClass(Base, [{
		...
		value: function initNumber() {
			for (var i = 1; i < 12; i++) {
				this.number.add(('' + i).padStart(2, '0')); //number是set对象,因为set对象的key值不重复
			}
		}
   ...
   */
		...
		value: function setOpenCode(code) {
			...
			self.updateOpenCode && self.opdateOpenCode.call(self.code);
		}
   ...
   */
		...
		value: function addCode() {
			...
			var count = _self.computeCount(active, _self.cur_play);
			if (count) {
				_self.addCodeItem($active.join(' '), _self.cur_play, _self.play_list.get(_self.cur_play).name, count);
			}
		}
   ...
   */
		...
		value: function addCodeItem(code, type, typeName, count) {
			...
			(0, _jquery2.default)(_self.cart_el).append(tpl);
			_self.getTotal();
		}
	}, {
		key: 'getCount',
   ...
   */
		...
		value: function getRandom(num) {
			var arr = [],
			    index = void 0;
			var number = Array.from(this.number); //this.number是一个set对象
			while (num--) {
		...
		value: function computeBonus(active, play_name) {
			...
			if (play[0] === 'r') {
				...
				if (play[1] - 5 > 0) {
					if (active - play[1] >= 0) {
						arr = new Array(active - 5).fill(0);
						max = Calculate.combine(arr, paly[1] - 5).length;
					} else {
						max = 0;
					}
				} else if (play[1] - 5 < 0) {
	...
	_createClass(Timer, [{
		...
		value: function countdown(end, update, handle) {
			...
			if (now - end > 0) {
				...
				_self.last_time = r.join('');
				update.call(_self, r.join(''));
				setTimeout(function () {
					_self.countdown(end, update, handle);
				}, 1000);
			}
		}
	}]);
